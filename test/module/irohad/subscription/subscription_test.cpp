...
TEST_F(SubscriptionTest, RepeatCopyControl) {
  ...
  manager->dispatcher()->repeat(
      0ull, std::chrono::milliseconds(0), [&work, &counter, t(std::move(t))]() {
        ++counter;
        if (!work)
...
TEST_F(SubscriptionTest, RepeatCounter) {
  ...
  manager->dispatcher()->repeat(
      0ull, std::chrono::microseconds(0), [&]() {
        ++counter;
        if (!work)
          throw std::runtime_error("Unexpected execution.");
      }, 
...
/**
 * @given subscription engine
 * @when put task that must repeat N times with 10ms delay untill counter less than 5
 * @and check it in predicate
...
TEST_F(SubscriptionTest, RepeatNTimes) {
  ...
  manager->dispatcher()->repeat(
      0ull, std::chrono::milliseconds(10), [&]() {
        ++counter;
        if (!work)
          throw std::runtime_error("Unexpected execution.");
      }, 
