  ...
  class AsyncDispatcher final : public IDispatcher,
   ...
   private:
    ...
    void uploadToHandler(typename Parent::Tid const tid,
                         ...
                         typename Parent::Task &&task, typename Parent::Predicate &&pred) {
      ...
      if (tid < kHandlersCount) {
        pred ?
        handlers_[tid].handler->repeat(timeout, std::move(task), std::move(pred)):
      ...
      if (auto context =
        ...
        pred ?
        context->handler->repeat(timeout, std::move(task), std::move(pred)):
   ...
   public:
    ...
    void add(typename Parent::Tid tid, typename Parent::Task &&task) override {
      uploadToHandler(tid, std::chrono::microseconds(0ull), std::move(task), nullptr);
    }
    ...
    void repeat(typename Parent::Tid tid, std::chrono::microseconds timeout, typename Parent::Task &&task, typename Parent::Predicate &&pred) override {
